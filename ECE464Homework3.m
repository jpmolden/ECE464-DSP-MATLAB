...
h=get(fh,'children');
h(3).YLim = [-25 50]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.
h(2).YLim = [-8 5];
...
%     fh = figure('NumberTitle', 'off', 'Name', 'Single Pole at the Origin');
%     FuncPlottingZSystemFunction(1, [0 -1],[1]) 
%     h=get(fh,'children');
%     h(3).YLim = [-10 10]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.
...
h=get(fh,'children');
h(3).YLim = [-40 20]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.
%% Junk
