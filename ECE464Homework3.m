...
FuncPlottingZSystemFunction([1 -1.5 -1], [1 0.9],'/',[1 -1], [1 0.7j], [1 -0.7j])
h=get(fh,'children')
...
FuncPlottingZSystemFunction(NumZ,'/',DenZ)    
    h=get(fh,'children');
...
FuncPlottingZSystemFunction([0 1],'/',[1 -0.7*exp(j*pi/4)]) 

fh = figure('NumberTitle', 'off', 'Name', 'Problem 2 c');
FuncPlottingZSystemFunction([1 -(1/0.7)*exp(j*pi/4)],'/', [1 -0.7*exp(j*pi/4)]) 
    h=get(fh,'children');
    h(4).YLim = [-10 10]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.


fh = figure('NumberTitle', 'off', 'Name', 'Problem 2 d');
FuncPlottingZSystemFunction([1 -(1/0.7)*exp(j*pi/4)],[0 -1] ,'/', [1 -0.7*exp(j*pi/4)])  
    h=get(fh,'children');
    h(4).YLim = [-10 10]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.
...
FuncPlottingZSystemFunction([(0.5-0.5j) -1],[(0.5+0.5j) -1],'/',[1 (-0.5-0.5j)], [1 (-0.5+0.5j)])
    h=get(fh,'children');
    h(4).YLim = [-10 10]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.

fh= figure('NumberTitle', 'off', 'Name', 'Problem 3 inv');
FuncPlottingZSystemFunction([1 (-0.5-0.5j)], [1 (-0.5+0.5j)],'/',[(0.5-0.5j) -1],[(0.5+0.5j) -1])
    h=get(fh,'children');
    h(4).YLim = [-10 10]; %Sets the subplot for magnitude to a sensable range to avoid quantization noise.
